(() => {
	...
	addEventListener("message", (event) => {
		...
		TurboMeshWebSocket._sockets = new Map();
		const sockets = TurboMeshWebSocket._sockets || TurboMeshWebSocket._sockets;
		...
		if (data.type === "turbomesh-ws-message") {
			const message = new MessageEvent("message", {
				data:
					data.opcode === 2
						? decodeBytes(data.body || "").buffer
						: decodeText(data.body || ""),
			});
		...
		if (
			data.type === "turbomesh-ws-close" ||
			data.type === "turbomesh-ws-error"
		) {
			socket.readyState = 3;
